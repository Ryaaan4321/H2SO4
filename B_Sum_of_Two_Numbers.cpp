...
namespace __DEBUG_UTIL__ {
    ...
    template <size_t N> void debug(const array<array<int, 2>, N>& container) { cout << "["; for (size_t i = 0; i < N; ++i) cout << "{" << container[i][0] << ", " << container[i][1] << "}" << (i != N - 1 ? ", " : ""); cout << "]" << endl; }
}
...
vector<int> digitofthenum(int n) {
    vector<int> dig;
    while (n != 0) {
        int tmp = n % 10;  
        dig.push_back(tmp); 
...
{
   int n;cin>>n;
   vector<int> ans=digitofthenum(n);
   debug(ans);
